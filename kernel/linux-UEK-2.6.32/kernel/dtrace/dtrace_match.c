...
printk(KERN_DEBUG "In match(...) hash %p best %d\n", hash, best);
	if (hash == NULL) {
...
printk(KERN_DEBUG "In match(...) probe %p\n", dtrace_hash_lookup(hash, &template));
	for (probe = dtrace_hash_lookup(hash, &template); probe != NULL;
	     probe = *(DTRACE_HASHNEXT(hash, probe))) {
printk(KERN_DEBUG "In match(...) Checking probe %p (%s, %s, %s, %s)\n", probe, probe->dtpr_provider->dtpv_name, probe->dtpr_mod, probe->dtpr_func, probe->dtpr_name);
		if (dtrace_match_probe(probe, pkp, priv, uid) <= 0)
			continue;
printk(KERN_DEBUG "In match(...) Possible matching probe %p\n", probe);
...
printk(KERN_DEBUG "probekey(%d, '%s', %p, '%s', %p, '%s', %p, '%s', %p)\n",
       pkp->dtpk_id, pkp->dtpk_prov, pkp->dtpk_pmatch, pkp->dtpk_mod,
       pkp->dtpk_mmatch, pkp->dtpk_func, pkp->dtpk_fmatch, pkp->dtpk_name,
       pkp->dtpk_nmatch);
}
