...
//NEED:
//dtrace_fulword(), dtrace_fuword32(), fasttrap_sulword(), fasttrap_suword32()
//fasttrap_fulword(), fasttrap_fuword32()
// replacement for struct regs;
static uint64_t
...
int
fasttrap_tracepoint_init(struct task_struct *p, fasttrap_tracepoint_t *tp,
			uintptr_t pc, fasttrap_probe_type_t type)
{
	 ...
	 */
	memcpy(tp->ftt_instr, instr, FASTTRAP_MAX_INSTR_SIZE);
...
//NEED: uread(), uwrite()//
int
fasttrap_tracepoint_install(struct task_struct *p, fasttrap_tracepoint_t *tp)
{
...
int
fasttrap_tracepoint_remove(struct task_struct *p, fasttrap_tracepoint_t *tp)
{
	uint8_t instr;
...
static void
fasttrap_sigsegv(struct task_struct *p, kthread_t *t, uintptr_t addr)
{
	struct siginfo si;
...
//	sqp->sq_info.si_addr = (caddr_t)addr;
	...
	mutex_lock(&p->dtrace_lock);
	///sigaddqa(p, t, sqp);
	sig_info_add(&si, p, t);
	mutex_unlock(&p->dtrace_lock);
...
fasttrap_pid_probe(struct regs *rp)
{
	...
	case FASTTRAP_T_RET16:
	{
		...
		if (ret == -1) {
			fasttrap_sigsegv(p, NULL, rp->r_sp);
			new_pc = pc;
			break;
		}
	...
	case FASTTRAP_T_COMMON:
	{
		...
		p->t_dtrace_scrpc = addr;
		memcpy(&scratch[i], tp->ftt_instr, tp->ftt_size);
		i += tp->ftt_size;
		...
		if (tp->ftt_ripmode != 0) {
			...
			case FASTTRAP_RIP_1:
				reg = &rp->r_rax;
				p->t_dtrace_reg = REG_RAX;
				break;
			case FASTTRAP_RIP_2:
				reg = &rp->r_rcx;
				p->t_dtrace_reg = REG_RCX;
				break;
			case FASTTRAP_RIP_1 | FASTTRAP_RIP_X:
				reg = &rp->r_r8;
				p->t_dtrace_reg = REG_R8;
				break;
			case FASTTRAP_RIP_2 | FASTTRAP_RIP_X:
				reg = &rp->r_r9;
				p->t_dtrace_reg = REG_R9;
				break;
			}
		...
		if (tp->ftt_retids != NULL) {
			p->t_dtrace_step = 1;
			p->t_dtrace_ret = 1;
			new_pc = p->t_dtrace_astpc;
		} else {
			new_pc = p->t_dtrace_scrpc;
		}

		p->t_dtrace_pc = pc;
		p->t_dtrace_npc = pc + tp->ftt_size;
		p->t_dtrace_on = 1;
		break;
	}
	...
	if (tp->ftt_retids != NULL) {
		...
		if (tp->ftt_type != FASTTRAP_T_COMMON) {
			...
			ASSERT(p->t_dtrace_ret != 0);
			ASSERT(p->t_dtrace_pc == pc);
			ASSERT(p->t_dtrace_scrpc != 0);
			ASSERT(new_pc == p->t_dtrace_astpc);
		}
	}
...
{
	struct task_struct *p = curproc;
	uintptr_t pc = p->t_dtrace_pc;
	uintptr_t npc = p->t_dtrace_npc;
			...
			int aframes)
{
	...
	return (fasttrap_anarg(regs, 1, argno));
}
			...
			int aframes)
{
	...
	return (fasttrap_anarg(regs, 0, argno));
}
...
fasttrap_getreg(struct regs *rp, uint_t reg)
{
	...
	case REG_FSBASE:	return (native_read_msr(MSR_AMD_FSBASE));
	case REG_GSBASE:	return (native_read_msr(MSR_AMD_GSBASE));
	}
