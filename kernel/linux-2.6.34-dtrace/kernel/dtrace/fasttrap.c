...
#include <linux/fasttrap_impl.h>
#include <linux/workqueue.h>
#include <asm/fasttrap_isa.h>
...
#define TIMEOUT_DISABLED	1	/* disable while detaching */
...
static int fasttrap_tracepoint_enable(struct task_struct *,
				fasttrap_probe_t *, uint_t);
static void fasttrap_tracepoint_disable(struct task_struct *,
				fasttrap_probe_t *, uint_t);
 ...
 */
void sigaddqa(proc_t *p, kthread_t *t, sigqueue_t *sigqp)
{
...
/*
 * sig_info_add() uses send_sig_info() in Linux to send a signal.
 * It lets __send_signal() allocate the sigqueue.
 *
 * If signal is ignored, discard but guarantee KILL and generation semantics.
 * It is called from sigqueue() and other places.
 * [Solaris uses sigaddqa() here instead.]
 */
void sig_info_add(struct siginfo *si, proc_t *p, kthread_t *t)
{
	int sig = si->si_signo;
...
void
fasttrap_sigtrap(struct task_struct *tsk, kthread_t *t, uintptr_t pc)
{
	//sigqueue_t *sqp = kzalloc(sizeof(sigqueue_t), __GFP_NOFAIL);
	...
	si.si_signo = SIGTRAP;
	si.si_errno = 0;
	si.si_code = TRAP_DTRACE;	/* TBD: add to header file */
	si.si_pid = task_pid_vnr(current);
	si.si_uid = current_uid();
	si.si_addr = (void __user *)pc;
	/* looks like si_addr overwrites pid/uid, so maybe pid/uid
	   are not needed */

	mutex_enter(&tsk->dtrace_lock);
	sig_info_add(&si, tsk, t);
	mutex_exit(&tsk->dtrace_lock);
...
fasttrap_pid_cleanup_cb(void *data)
{
		...
		for (i = 0; i < fasttrap_provs.fth_nent; i++) {
			...
			while ((fp = *fpp) != NULL) {
				...
				if (dtrace_unregister(provid) != 0) {
					if (atomic_read(&fasttrap_total) > fasttrap_max / 2)
	 ...
	 * (if detach fails).
	 */
	// TBD: can fasttrap_timeout == TIMEOUT_ACTIVE here?
	if (later > 0 && fasttrap_timeout != (timeout_id_t)TIMEOUT_DISABLED) {
		///fasttrap_timeout = timeout(&fasttrap_pid_cleanup_cb, NULL, HZ);
		INIT_DELAYED_WORK(pidwork, &fasttrap_pid_cleanup_cb);
		if (!schedule_delayed_work(pidwork, HZ))
			pr_warning(
				"%s: schedule_delayed_work: already queued\n",
				__func__);
	} else if (later > 0)
...
fasttrap_pid_cleanup(void)
{
	...
	if (fasttrap_timeout == 0) {
		...
		INIT_DELAYED_WORK(pidwork, &fasttrap_pid_cleanup_cb);
		if (!schedule_delayed_work(pidwork, 1))
			pr_warning(
				"%s: schedule_delayed_work: already queued\n",
				__func__);
		fasttrap_timeout = TIMEOUT_ACTIVE;
	}
	mutex_unlock(&fasttrap_cleanup_mtx);
}
...
fasttrap_fork(proc_t *p, proc_t *cp)
{
	...
	ASSERT(p->dtrace_count > 0);
	ASSERT(cp->dtrace_count == 0);
	 ...
	 */
	mutex_enter(&cp->dtrace_lock);
	sprlock_proc(cp);
	mutex_exit(&cp->dtrace_lock);
...
{
	ASSERT(p == curproc);
	ASSERT(mutex_is_locked(&p->dtrace_lock));

	mutex_exit(&p->dtrace_lock);
...
static int
fasttrap_tracepoint_enable(struct task_struct *tsk,
				fasttrap_probe_t *probe, uint_t index)
{
...
again:
	...
	if (new_tp != NULL) {
		 ...
		 */
		if (fasttrap_tracepoint_install(tsk, new_tp) != 0)
		 ...
		 * the victim process.
		 */
		ASSERT(tsk->p_proc_flag & P_PR_LOCK);
		tsk->dtrace_count++;
	 ...
	 */
	if (fasttrap_tracepoint_init(tsk, new_tp, pc, id->fti_ptype) == 0)
		goto again;
...
static void
fasttrap_tracepoint_disable(struct task_struct *tsk,
				fasttrap_probe_t *probe, uint_t index)
{
	fasttrap_bucket_t *bucket;
	...
	*idp = id->fti_next;
	...
	if (tsk != NULL) {
		 ...
		 * state.
		 */
		if (fasttrap_tracepoint_remove(tsk, tp) != 0)
			fasttrap_sigtrap(tsk, NULL, pc);
...
fasttrap_pid_enable(void *arg, dtrace_id_t id, void *parg)
{
	 ...
	 */
	if ((tsk = sprlock(probe->ftp_pid)) == NULL) {
		if ((curproc->p_flag & SFORKING) == 0)
			return (0);

		mutex_enter(&pidlock);
		tsk = find_process_by_pid(probe->ftp_pid);
		 ...
		 */
		ASSERT(tsk != NULL);
		ASSERT(tsk->p_parent == curproc);
		ASSERT(tsk->p_stat == SIDL);
	...
	for (i = 0; i < probe->ftp_ntps; i++) {
		if ((rc = fasttrap_tracepoint_enable(tsk, probe, i)) != 0) {
			/*
			 ...
			 */
			while (i >= 0) {
				fasttrap_tracepoint_disable(tsk, probe, i);
				i--;
			}
...
fasttrap_pid_destroy(void *arg, dtrace_id_t id, void *parg)
{
	...
	atomic_add_32(-probe->ftp_ntps, &fasttrap_total);
...
fasttrap_proc_lookup(pid_t pid)
{
	...
	for (fprc = bucket->ftb_data; fprc != NULL; fprc = fprc->ftpc_next) {
		if (fprc->ftpc_pid == pid && fprc->ftpc_acount != 0) {
			...
			mutex_unlock(&bucket->ftb_mtx);
			fprc->ftpc_rcount++;
			atomic_long_add(1, &fprc->ftpc_acount);
	...
	new_fprc = kzalloc(sizeof(fasttrap_proc_t), __GFP_NOFAIL);
			...
			const dtrace_pattr_t *pattr)
{
	 ...
	 */
	mutex_enter(&pidlock);
	if ((tsk = find_process_by_pid(pid)) == NULL) {
		mutex_exit(&pidlock);
		return (NULL);
	}
	mutex_enter(&tsk->dtrace_lock);
	mutex_exit(&pidlock);
	if (tsk->p_flag & (SVFORK | SEXITING)) {
		mutex_exit(&tsk->dtrace_lock);
		return (NULL);
	}
	 ...
	 */
	mutex_enter(&tsk->p_crlock);
	crhold(tsk->cred);	// TBD: or real_cred?
	cred = (struct cred *)tsk->cred;
	mutex_exit(&tsk->p_crlock);
	mutex_exit(&tsk->dtrace_lock);

	new_fp = kzalloc(sizeof(fasttrap_provider_t), __GFP_NOFAIL);
	new_fp->ftp_pid = pid;
...
fasttrap_provider_free(fasttrap_provider_t *provider)
{
	...
	if (!provider->ftp_retired) {
		atomic_long_add(-1, &provider->ftp_proc->ftpc_acount);
...
fasttrap_provider_retire(pid_t pid, const char *name, int mprov)
{
	 ...
	 * table.
	 */
	atomic_long_add(-1, &fp->ftp_proc->ftpc_acount);
...
static void
fasttrap_uint32_swap(const void *ap, const void *bp, size_t size)
{
	u32 t = *(u32 *)ap;
	*(u32 *)ap = *(u32 *)bp;
	*(u32 *)bp = t;
...
static void
fasttrap_uint64_swap(const void *ap, const void *bp, size_t size)
{
	u64 t = *(u64 *)ap;
	*(u64 *)ap = *(u64 *)bp;
	*(u64 *)bp = t;
...
fasttrap_add_probe(fasttrap_probe_spec_t *pdata)
{
	...
	if (name == NULL) {
		for (i = 0; i < pdata->ftps_noffs; i++) {
			...
			atomic_add_32(1, &fasttrap_total);
			...
			pp = kzalloc(sizeof(fasttrap_probe_t), __GFP_NOFAIL);
			...
			tp = kzalloc(sizeof(fasttrap_tracepoint_t),
				__GFP_NOFAIL);
		...
		atomic_add_32(pdata->ftps_noffs, &fasttrap_total);
		 ...
		 * for a given pc.
		 */
		sort(pdata->ftps_offs, pdata->ftps_noffs, sizeof(uint64_t),
			fasttrap_uint64_cmp, fasttrap_uint64_swap);
		...
		pp = kzalloc(offsetof(fasttrap_probe_t,
			ftp_tps[pdata->ftps_noffs]), __GFP_NOFAIL);
...
fasttrap_meta_provide(void *arg, dtrace_helper_provdesc_t *dhpv, pid_t pid)
{
	...
	if (strlen(dhpv->dthpv_provname) + 10 >=
		...
		pr_warning("failed to instantiate provider %s: "
	...
	if ((provider = fasttrap_provider_lookup(pid, dhpv->dthpv_provname,
		...
		pr_warning("failed to instantiate provider %s for "
			...
			dtrace_helper_probedesc_t *dhpb)
{
	...
	sort(dhpb->dthpb_offs, dhpb->dthpb_noffs, sizeof(uint32_t),
		fasttrap_uint32_cmp, fasttrap_uint32_swap);
	...
	atomic_add_32(ntps, &fasttrap_total);
	...
	pp = kzalloc(offsetof(fasttrap_probe_t, ftp_tps[ntps]), __GFP_NOFAIL);
	...
	for (i = 0; i < dhpb->dthpb_noffs; i++) {
		tp = kzalloc(sizeof(fasttrap_tracepoint_t), __GFP_NOFAIL);
...
fasttrap_ioctl(dev_t dev, int cmd, intptr_t arg, int md, cred_t *cr, int *rv)
{
	...
	if (cmd == FASTTRAPIOC_MAKEPROBE) {
		...
		if (copy_from_user(&noffs, &uprobe->ftps_noffs,
		...
		if (!PRIV_POLICY_CHOICE(cr, PRIV_ALL, B_FALSE)) {
			...
			if ((ret = priv_proc_cred_perm(cr, tsk, NULL,
			    VREAD | VWRITE)) != 0) {
				mutex_exit(&tsk->dtrace_lock);
				return (ret);
			}
...
err:
		...
		if (copy_from_user(&instr, (void *)arg, sizeof(instr)) != 0)
			return (-EFAULT);
		...
		mutex_lock(&fasttrap_tpoints.fth_table[index].ftb_mtx);
		...
		if (tp == NULL) {
			mutex_unlock(&fasttrap_tpoints.fth_table[index].ftb_mtx);
			return (-ENOENT);
		}

		memcpy(&instr.ftiq_instr, &tp->ftt_instr,
			sizeof(instr.ftiq_instr));
		mutex_unlock(&fasttrap_tpoints.fth_table[index].ftb_mtx);

		if (copy_to_user((void *)arg, &instr, sizeof(instr)) != 0)
			return (-EFAULT);
...
fasttrap_attach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	fasttrap_tpoints.fth_table = kzalloc(fasttrap_tpoints.fth_nent *
		sizeof(fasttrap_bucket_t), __GFP_NOFAIL);
	...
	fasttrap_provs.fth_table = kzalloc(fasttrap_provs.fth_nent *
		sizeof(fasttrap_bucket_t), __GFP_NOFAIL);
	...
	fasttrap_procs.fth_table = kzalloc(fasttrap_procs.fth_nent *
		sizeof(fasttrap_bucket_t), __GFP_NOFAIL);
...
fasttrap_detach(dev_info_t *devi, ddi_detach_cmd_t cmd)
{
	 ...
	 * to a non-0 value (=1), and wait for the current timeout to complete.
	 */
	mutex_lock(&fasttrap_cleanup_mtx);
	fasttrap_cleanup_work = 0;

	while (fasttrap_timeout != (timeout_id_t)TIMEOUT_DISABLED) {
		tmp = fasttrap_timeout;
		fasttrap_timeout = (timeout_id_t)TIMEOUT_DISABLED;

		if (tmp != 0) {
			mutex_unlock(&fasttrap_cleanup_mtx);
			///(void) untimeout(tmp);
			cancel_delayed_work_sync(pidwork);
			mutex_lock(&fasttrap_cleanup_mtx);
		}
	}
...
{
	// register what/where?
	// have a verbose_ioctl flag in sysfs for ioctl logging?
	// see mm/vmstat.c for example;
	return 0;
}
