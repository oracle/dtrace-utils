...
translator bufinfo_t < struct bio *B > {
	...
	b_error = 0;
	b_edev = B->bi_disk == NULL ? 0 : B->bi_disk->part_tbl->part[B->bi_partno]->__dev.devt;
};
...
translator devinfo_t < struct bio *B > {
	dev_major = B->bi_disk == NULL ? 0 : getmajor(B->bi_disk->part_tbl->part[B->bi_partno]->__dev.devt);
	dev_minor = B->bi_disk == NULL ? 0 : getminor(B->bi_disk->part_tbl->part[B->bi_partno]->__dev.devt);
	dev_instance = 0;
