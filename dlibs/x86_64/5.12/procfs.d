...
translator lwpsinfo_t < struct task_struct *T > {
	pr_flag = (	T->state & 0x0004) ? PR_STOPPED : 0;
/*
...
*/
	...
	pr_state = (	T->state & 0x0001) ? SSLEEP :
		   (	T->state & 0x0002) ? SSLEEP :
		   (	T->state & 0x0004) ? SSTOP :
		   (	T->state & 0x0008) ? SSTOP :
		   (	T->state & 0x0020) ? SZOMB :
		   (	T->state & 0x0010) ? SZOMB :
		   (	T->state & 0x0080) ? SZOMB :
		   (	T->state & 0x0100) ? SWAIT :
		   (	T->state & 0x0200) ? SWAIT : SRUN;
	pr_sname = (	T->state & 0x0001) ? 'S' :
		   (	T->state & 0x0002) ? 'S' :
		   (	T->state & 0x0004) ? 'T' :
		   (	T->state & 0x0008) ? 'T' :
		   (	T->state & 0x0020) ? 'Z' :
