...
translator lwpsinfo_t < struct task_struct *T > {
	pr_flag = (T->__state & 0x0004) ? PR_STOPPED : 0;
/*
...
*/
	...
	pr_state = (T->__state & 0x0001) ? SSLEEP :
		   (T->__state & 0x0002) ? SSLEEP :
		   (T->__state & 0x0004) ? SSTOP :
		   (T->__state & 0x0008) ? SSTOP :
		   (T->__state & 0x0020) ? SZOMB :
		   (T->__state & 0x0010) ? SZOMB :
		   (T->__state & 0x0080) ? SZOMB :
		   (T->__state & 0x0100) ? SWAIT :
		   (T->__state & 0x0200) ? SWAIT : SRUN;
	pr_sname = (T->__state & 0x0001) ? 'S' :
		   (T->__state & 0x0002) ? 'S' :
		   (T->__state & 0x0004) ? 'T' :
		   (T->__state & 0x0008) ? 'T' :
		   (T->__state & 0x0020) ? 'Z' :
