...
Pabort_agent(struct ps_prochandle *P)
{
	...
	_dprintf("agent LWP is asleep in syscall %d\n", sysnum);
	(void) Pstop(P, 0);
	...
	if (Psetrun(P, 0, PRSABORT) == 0) {
		...
		_dprintf("agent LWP system call aborted\n");
	}
}
...
Pcreate_agent(struct ps_prochandle *P)
{
	...
	if (P->status.pr_lwp.pr_flags & PR_ASLEEP) {
		_dprintf("Pcreate_agent: aborting agent syscall\n");
		Pabort_agent(P);
	}
...
Pdestroy_agent(struct ps_prochandle *P)
{
	if (P->agentcnt > 1)
		...
		if ((flags & (PR_AGENT|PR_ASLEEP)) == (PR_AGENT|PR_ASLEEP)) {
			_dprintf("Pdestroy_agent: aborting agent syscall\n");
			Pabort_agent(P);
		}
	...
	argdes_t *argp)		/* argument descriptor array */
{
	...
	_dprintf("Psyscall(): execute(sysindex = %d)\n", sysindex);
	...
	_dprintf("Psyscall(): copying arguments\n");
	 ...
	 */
	_dprintf("Psyscall(): set running at sysentry\n");
	...
	if (!Pissyscall_prev(P, P->status.pr_lwp.pr_reg[R_PC], NULL)) {
		_dprintf("Pissyscall_prev() failed\n");
		goto bad24;
	}

	_dprintf("Psyscall(): caught at sysexit\n");
...
bad1:	Perr++;
	error = -1;
	_dprintf("Psyscall(%d) fails with local error %d\n", sysindex, Perr);
