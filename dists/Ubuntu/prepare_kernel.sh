...
# This script installs a kernel with features DTrace uses.
echo
echo "This script ($0) is meant to be run on an Ubuntu"
echo "installation, to facilitate building a Linux kernel with features helpful"
echo "for DTrace to operate on Ubuntu."
echo "To facilitate the use of DTrace, this script runs other scripts which:"
echo " -- run 'update' and 'upgrade' on the Ubuntu system"
echo " -- install packages (e.g. dependencies for the kernel build)"
echo " -- git-clone, build, and install binutils and gcc (providing bpf)"
echo " -- git-clone and build an Ubuntu kernel with additional tracing features"
...
echo
echo "This script builds a kernel with additional tracing features DTrace uses."
...
echo ""
echo "The required package installations, source code, and builds consume about"
echo "39G more storage space, when starting from a fresh install of Ubuntu."
