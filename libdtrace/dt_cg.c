...
/*
 * Generate a load for the specified node and CTF type.
 */
static void
dt_cg_load(dt_node_t *dnp, dt_irlist_t *dlp, ctf_file_t *ctfp,
    ctf_id_t type, int dst_reg, int src_reg)
{
	int bpf_size;
	...
	case 4: bpf_size = BPF_W; break;
	case 8: bpf_size = BPF_DW; break;
	}
	...
	dt_irlist_append(dlp, dt_cg_node_alloc(BPF_LDX_MEM(bpf_size,
		    dst_reg, src_reg, 0)));

	if ((dnp->dn_flags & DT_NF_SIGNED) && (size != 8)) {
		instr = BPF_ALU64_IMM(BPF_LSH, dst_reg, 64 - (size * 8));
		dt_irlist_append(dlp, dt_cg_node_alloc(instr));

		instr = BPF_ALU64_IMM(BPF_ARSH, dst_reg, 64 - (size * 8));
		dt_irlist_append(dlp, dt_cg_node_alloc(instr));
	}
}

static void
dt_cg_ptrsize(dt_node_t *dnp, dt_irlist_t *dlp, uint_t op, int dreg)
{
 ...
 * the diagrams below, the region above the hyphens is generated by dt_cg_load().
 *
 * Epilogue for unsigned fields:	Epilogue for signed fields:
 *
 * ldx	[r1], r1			ldx [r1], r1
 * ----					lsh r1, (64 - SIZE * 8)
 * lsh	r1, USHIFT	   	   	arsh r1, (64 - SIZE * 8)
 * and	r1, (1 << bits) - 1		----
 *    					lsh  r1, (64 - SSHIFT)
 * 					arsh  r1, 64 - bits
 *
 ...
 */
static void
dt_cg_field_get(dt_node_t *dnp, dt_irlist_t *dlp, ctf_file_t *fp,
    const ctf_membinfo_t *mp)
{
	ctf_encoding_t e;
	struct bpf_insn instr;
	uint64_t shift;
 ...
 * a scalar type is being narrowed or changing signedness.  We first shift the
    ...
    dt_regset_t *drp, uint_t op)
{
	...
	int divlike = (dnp->dn_op == DT_TOK_DIV || dnp->dn_op == DT_TOK_MOD ||
	    dnp->dn_op == DT_TOK_DIV_EQ || dnp->dn_op == DT_TOK_MOD_EQ);

	int signed_operands = ((dlp->dn_flags & DT_NF_SIGNED) ||
	    (drp->dn_flags & DT_NF_SIGNED));
	...
	if ((sign_bit = dt_regset_alloc(drp)) == -1)
		longjmp(yypcb->pcb_jmpbuf, EDT_NOREG);
	...
	if (is_ptr_op && rp_is_ptr)
		dt_cg_ptrsize(dnp, dlp, BPF_MUL, dnp->dn_left->dn_reg);

	dt_cg_node(dnp->dn_right, dlp, drp);
	if (is_ptr_op && lp_is_ptr)
		dt_cg_ptrsize(dnp, dlp, BPF_MUL, dnp->dn_right->dn_reg);

	/*
	 * Extract the sign bit by xoring together the args (if both signed:
	 * otherwise, just move whichever is signed into place) then masking.
	 */
	if (divlike && signed_operands) {
		int mask_sign;
		int mask_val;

		if (((mask_sign = dt_regset_alloc(drp)) == -1) ||
		    ((mask_val = dt_regset_alloc(drp)) == -1))
			longjmp(yycb->pcb_jmpbuf, EDT_NOREG);

		if (dlp->dn_flags & DT_NF_SIGNED) {
			dt_cg_mov(sign_bit, dnp->dn_left->dn_reg);

			if (drp->dn_flags & DT_NF_SIGNED) {
				instr = BPF_ALU64_REG(BPF_XOR, sign_bit,
				    dnp->dn_right->dn_reg);
				dt_irlist_append(dlp, dt_cg_node_alloc(instr));
			}
		} else {
			assert(drp->dn_flags & DT_NF_SIGNED);
			dt_cg_mov(sign_bit, dnp->dn_right->dn_reg);
		}

		/*
		 * mask_val ends up with the top bit zero.
		 * mask_sign ends up with only the top bit nonzero.
		 */
		dt_cg_setx(dlp, mask_val, ~0);
		instr = BPF_ALU64_IMM(BPF_LSH, mask_val, 1);
		dt_irlist_append(dlp, dt_cg_node_alloc(instr));
		instr = BPF_ALU64_IMM(BPF_RSH, mask_val, 1);
		dt_irlist_append(dlp, dt_cg_node_alloc(instr));

		dt_cg_setx(dlp, mask_sign, ~0);
		instr = BPF_ALU64_REG(BPF_XOR, mask_sign, mask_val);
		dt_irlist_append(dlp, dt_cg_node_alloc(instr));

		instr = BPF_ALU64_REG(BPF_AND, sign_bit, mask_sign);
		dt_irlist_append(dlp, dt_cg_node_alloc(instr));

		if (dlp->dn_flags & DT_NF_SIGNED) {
			instr = BPF_ALU64_REG(BPF_AND, dnp->dn_left->dn_reg,
			    mask_val);
			dt_irlist_append(dlp, dt_cg_node_alloc(instr));
		}

		if (drp->dn_flags & DT_NF_SIGNED) {
			instr = BPF_ALU64_REG(BPF_AND, dnp->dn_right->dn_reg,
			    mask_val);
			dt_irlist_append(dlp, dt_cg_node_alloc(instr));
		}

		dt_regset_free(drp, mask_sign);
		dt_regset_free(drp, mask_val);
	}
	...
	if (divlike && signed_operands) {
		instr = BPF_ALU64_REG(BPF_OR, dnp->dn_reg, sign_bit);
		dt_irlist_append(dlp, dt_cg_node_alloc(instr));
	}

	if (lp_is_ptr && rp_is_ptr)
		dt_cg_ptrsize(dnp->dn_right, dlp, BP_DIV, dnp->dn_reg);

	dt_regset_free(drp, sign_bit);
}
...
dt_cg_node(dt_node_t *dnp, dt_irlist_t *dlp, dt_regset_t *drp)
{
	...
	case DT_TOK_DEREF:
		...
		if (!(dnp->dn_flags & DT_NF_REF)) {
			...
			dt_cg_load(dnp, dlp, ctfp, dnp->dn_type,
	...
	case DT_TOK_DOT:
		...
		if (!(dnp->dn_flags & DT_NF_REF)) {
			...
			dt_cg_load(dnp, dlp, ctfp, m.ctm_type,
			...
			if (dnp->dn_flags & DT_NF_BITFIELD)
				dt_cg_field_get(dnp, dlp, ctfp, &m);
		}
	...
	case DT_TOK_IDENT:
		...
		case DT_NODE_SYM: {
			...
			if (!(dnp->dn_flags & DT_NF_REF))
				dt_cg_load(dnp, dlp, dnp->dn_type, dnp->dn_reg,
				    dnp->dn_reg);
			break;
		}
