...
DIRNAME="$tmpdir/libproc-bad-sh_entsize.$$.$RANDOM"
mkdir -p $DIRNAME
cd $DIRNAME
...
xxd -c8 $trigger | \
	gawk '/^00000028:/ {
	      ...
	      # Find the offset of the sh_entsize field in the symtab section
	      # header, output an 0-value to repalce it, and exit.
...
# If the problem exists, the dtrace invocation will trigger a FP exception.
...
$dtrace $dt_flags -xdebug -c ./bad-delaydie -qn 'pid$target:a.out::entry { @[probefunc] = count(); }' |& \
	grep "Pbuild_file_symtab: sh_entsize"
