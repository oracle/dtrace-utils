...
# At least three problems are known:
#
# 1. The most egregious is the incorrect assumption that symbols we are going
# to skip must belong to the previous module  The use of "last_dmp" in that
# function is wrong.
#
# 2. Another important issue is how to handle __key.* symbols.  E.g. consider
...
#
# 3. A small case that has caused confusion is
...
while [ `ls *@* | wc -l` -gt 0 ]; do
	# pick the first $modname@XXXX file in the directory
	cmds=`ls *@* | head -1`
	ncmds=`cat $cmds | wc --lines`

	# extract the module name
	modname=`echo $cmds | sed 's/@.*$//'`
